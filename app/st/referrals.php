...
?>
   <body>
      ...
      <div class="layout-wrapper layout-content-navbar  ">
         <div class="layout-container">
            ...
            require_once "content/referrals.php";
            ?>
               <!-- / Navbar -->
               <!-- Content wrapper -->
   ...
   <script>
    ...
    <script>
      function claim() {
            $.get('phpajax/claim', '', function(data){
               console.log(data);
                  if (data == 'failed1') {
                        notie.alert({ type: 'error', position: 'bottom', text: 'The task is incomplete', time: 10 });
                  }
                  if (data == 'failed2') {
                        notie.alert({ type: 'error', position: 'bottom', text: 'You have already claimed', time: 10 });
                  }
                  if (data == 'failed3') {
                        notie.alert({ type: 'error', position: 'bottom', text: 'Competition is closed', time: 10 });
                  }
                  if (data=='success') {
                     notie.alert({ type: 1, time: 60, text: '<b>You have</b><br>Claimed <b><h1 class=text-white>R150</h1></b>' })
                  }
            })
      }
  </script>
